...
import {normalizeDraft,effective,readiness,observation,lengthFor} from '../lib/model.mjs';
...
test('enqueue validates all destinations, snapshots content and prevents duplicate submissions',async t=>{const {e}=await fixture(t);let d=await e.save(sample());await assert.rejects(e.enqueue(d.id,d.revision),/要対応/);await connect(e);const jobs=await e.enqueue(d.id,d.revision);d=await e.save({...d,common:{body:'後で変更'}});assert.equal(jobs[0].content.body,'テスト本文');await assert.rejects(e.enqueue(d.id,d.revision),/すでに登録/);await assert.rejects(e.disconnect('facebook'),/予約/);});
...
test('RedNote official international URLs can be recorded; lookalike domains are rejected',async t=>{
 ...
 assert.equal(e.snapshot().jobs[0].status,'published');
});
...
test('immediate preflight validates every destination and paused state before registering any job',async t=>{
 const f=await fixture(t);await connect(f.e);let d=await f.e.save({...sample(),scheduledAt:'',selected:['facebook','threads']});await assert.rejects(f.e.enqueue(d.id,d.revision,true),/要対応/);assert.equal(f.e.snapshot().jobs.length,0);
 d=await f.e.save({...d,selected:['facebook']});await f.e.settings({...f.e.snapshot().settings,paused:true});await assert.rejects(f.e.enqueue(d.id,d.revision,true),/再開/);assert.equal(f.e.snapshot().jobs.length,0);assert.equal(f.publishes(),0);
