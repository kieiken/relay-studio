...
export async function createStudio({dataDir=process.env.RELAY_DATA_DIR??path.join(os.homedir(),'Library','Application Support','Relay Studio'),credentials,connectors,now,worker=true}={}) {
 ...
 const server=http.createServer(async(req,res)=>{try{
  ...
  check(!req.headers.origin||req.headers.origin===`http://${req.headers.host}`,'このアプリの画面から操作してください。',403);
  check(!['cross-site','same-site'].includes(req.headers['sec-fetch-site']),'外部サイトからの接続はできません。',403);
  ...
  if(route.startsWith('/api/')&&method==='POST'){
   ...
   let bytes=0,chunks=[];for await(const chunk of req){bytes+=chunk.length;check(bytes<=1024*1024,'データが大きすぎます。',413);chunks.push(chunk);}
   ...
   const c=/^\/api\/connections\/(facebook|threads|instagram)\/(connect|disconnect)$/.exec(route);
   ...
   const j=/^\/api\/jobs\/([a-f0-9-]+)\/(cancel|record|feedback|reconcile)$/.exec(route);
   if(j)return send(res,200,await engine[j[2]](j[1],body));
  }
  if(method==='GET'&&['/','/index.html','/app.js','/styles.css','/catalog.js','/i18n.js','/messages-zh-TW.js','/favicon.svg'].includes(route)){
