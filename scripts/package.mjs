...
try{
 for(const entry of ['extension','public','lib','live','node_modules','server.mjs','package.json','package-lock.json','README.md','DESIGN-REVIEW.md','INSTALL_WITH_AI.md','RELEASE-NOTES.md','start.command'])await cp(path.join(root,entry),path.join(folder,entry),{recursive:true});
 await mkdir(path.join(folder,'tests'));await mkdir(path.join(folder,'scripts'));
 for(const test of ['extension-recovery.test.mjs','browser-dom.test.mjs','browser-bridge.test.mjs','studio.test.mjs','connectors.test.mjs','i18n.test.mjs','install.test.mjs'])await cp(path.join(root,'tests',test),path.join(folder,'tests',test));
