...
export class Engine {
 constructor({dataDir,credentials,connectors=new Connectors(),now=()=>new Date()}={}) {this.dir=dataDir;this.file=path.join(dataDir,'studio.json');this.credentials=credentials;this.connectors=connectors;this.now=now;this.tail=Promise.resolve();}
 async init(){
  ...
  for(const j of this.state.jobs){if(j.status==='sending'){j.status='unknown';j.reason='送信中にアプリが終了しました。公開先を照合してください。';}if(j.feedbackAttempt==='collecting'){j.feedbackAttempt='unavailable';j.feedbackError='取得処理が中断しました。公式画面で確認してください。';}}
 ...
 async disconnect(id){return this.exclusive(async()=>{check(ids.includes(id),'媒体が不正です。');check(!this.state.jobs.some(j=>j.platform===id&&['queued','sending','unknown'].includes(j.status)),'予約を取り消し、未確認の結果を整理してから接続解除してください。',409);await this.credentials.delete(id);delete this.state.connections[id];await this.persist();return this.snapshot();});}
 ...
 async enqueue(id,revision,sendNow=false){return this.exclusive(async()=>{const d=this.state.drafts.find(d=>d.id===id);check(d&&d.revision===revision,'最新の下書きを保存してから登録してください。',409);check(d.selected.length>0,'投稿先を一つ以上選択してください。');const ready=readiness(d,this.state.connections);check(ready.every(r=>!r.issues.length),'投稿先ごとの要対応項目を解消してください。',409);check(typeof sendNow==='boolean','投稿方法が正しくありません。');
  ...
  const jobs=d.selected.map(p=>({id:randomUUID(),draftId:id,revision,platform:p,content:structuredClone(effective(d,p)),accountId:this.state.connections[p]?.accountId??null,scheduledAt,status:platforms.find(v=>v.id===p).auto?'queued':'manual',createdAt:this.now().toISOString(),observations:[]}));this.state.jobs.push(...jobs);await this.persist();return jobs;});}
 ...
 async reconcile(id){return this.exclusive(async()=>{const j=this.job(id);check(j.status==='unknown'&&j.remoteId,'自動照合には公開先IDが必要です。公式画面で確認してください。',409);const r=await this.connectors.inspect(j,await this.credentials.get(j.platform));Object.assign(j,r,{status:'published',verifiedBy:'official-api',reason:null});await this.persist();return this.snapshot();});}
 async tick(){return this.exclusive(async()=>{if(this.state.settings.paused)return;
  for(const j of this.state.jobs.filter(j=>j.status==='queued'&&Date.parse(j.scheduledAt)<=this.now().getTime()).slice(0,6)){
