...
export function content(input={}) {
  ...
  c.attachmentId=str(input.attachmentId??'',36);check(!c.attachmentId||/^[a-f0-9-]{36}$/.test(c.attachmentId),'画像IDが正しくありません。');
...
export function readiness(draft,connections) {
 ...
 if(id==='facebook' && (c.mediaUrl||c.attachmentId)) issues.push('初版のFacebook自動配信はテキストのみ。個別原稿で画像URLを外してください');
 if(id==='threads' && (c.mediaUrl||c.attachmentId)) issues.push('初版のThreads自動配信はテキストのみ。個別原稿で画像URLを外してください');
 if(['note','rednote'].includes(id)&&!c.title.trim()) issues.push('タイトルを入力');
 if(id==='rednote'&&[...c.title].length>20) issues.push('タイトルは20文字以内');
 if(id==='rednote'&&!c.attachmentId) issues.push('画像ファイルを選択してください');
 if(p.browser&&c.mediaUrl&&!c.attachmentId) issues.push('ブラウザ投稿の画像はファイルを選択してください');
 if((p.auto||p.browser) && !connections[id]?.verifiedAt) issues.push('接続テストが必要');
 return {id,mode:p.auto?'auto':'browser',issues,count:lengthFor(id,text),limit:p.limit}; });
}
