...
export function verifyBrowserEvidence(job,input,now){
 ...
 return {permalink:url.href,remoteId:job.platform==='rednote'?m[1]:m[2],publishedAt:new Date(at).toISOString(),verifiedBy:'browser-visible-content'};
}
export class BrowserBridge {
 ...
 async connect(input){return this.e.exclusive(async()=>{
  ...
  this.e.state.connections[input.platform]={transport:'browser',accountId:input.accountId,name:String(input.name||input.accountId).slice(0,100),verifiedAt:this.e.now().toISOString()};await this.e.persist();return {connected:true};
 });}
 ...
 async feedbackWork(){return this.e.exclusive(async()=>{
  ...
  const j=this.e.state.jobs.find(j=>j.transport==='browser'&&j.status==='published'&&j.feedbackAttempt!=='collecting'&&(j.feedbackRequestedAt||!j.feedbackAttempt&&+this.e.now()-Date.parse(j.publishedAt)>=48*3600000));
 ...
 async feedbackResult(input){return this.e.exclusive(async()=>{
  ...
  if(input.evidence){check(input.evidence.accountId===j.accountId&&input.evidence.permalink===j.permalink,'反響取得先が一致しません。');const at=this.e.now();const o=observation({metrics:input.metrics,measuredAt:at.toISOString(),period:(at-Date.parse(j.publishedAt))/3600000>=48&&(at-Date.parse(j.publishedAt))/3600000<=54?'publication_48h':'current_lifetime',reference:j.permalink},j,at);o.source='browser-visible-content';j.observations.push(o);j.feedbackAttempt='collected';}
