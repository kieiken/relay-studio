...
export class BrowserBridge {
 ...
 async feedbackResult(input){return this.e.exclusive(async()=>{
  ...
  if(input.evidence){check(input.evidence.accountId===j.accountId&&input.evidence.permalink===j.permalink,'反響取得先が一致しません。');const at=this.e.now();const reference=j.platform==='note'?'https://note.com/dashboard':j.permalink;check(!input.reference||input.reference===reference,'反響取得先が一致しません。');const o=observation({metrics:input.metrics,measuredAt:at.toISOString(),period:at-Date.parse(j.publishedAt)>=48*3600000+(j.timePrecision==='minute'?60000:0)&&(at-Date.parse(j.publishedAt))/3600000<=54?'publication_48h':'current_lifetime',reference},j,at);o.source='browser-visible-content';j.observations.push(o);j.feedbackAttempt='collected';}
